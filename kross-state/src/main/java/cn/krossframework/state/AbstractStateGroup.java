...
public abstract class AbstractStateGroup implements StateGroup {
	...
	@Override public Long getCurrentWorkerId() {
		return this.currentWorkerId;
	}

	@Override @SuppressWarnings("unchecked") public <T extends StateGroupConfig> T getConfig() {
		return (T) this.stateGroupConfig;
	}

	@Override @SuppressWarnings("unchecked") public <T extends StateData> T getStateData() {
		return (T) this.stateData;
	}

	@Override public void setCurrentWorkerId(long id) {
	...
	@Override public void initAndSetCurrentState(String code) throws NullPointerException {
	...
	@Override public boolean tryEnterGroup(Task task) {
		return !this.canDeposed() && !this.stateData.isFull();
	}

	@Override public boolean tryAddTask(Task task) {
	...
	@Override public boolean canDeposed() {
		return (this.currentState == null || DefaultErrorState.CODE
				.equals(this.currentState.getCode())) && this.getStateData().isEmpty();
	}
}
